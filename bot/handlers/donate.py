...
from bot.states.donate import DonateState
...
async def handle_donate(callback: CallbackQuery, state: FSMContext):
    kb = await get_donate_kb()
    await callback.message.edit_caption(caption=start_donate_text, parse_mode="None", reply_markup=kb)
    await state.set_state(DonateState.donate)


@donate_router.callback_query(DonateCallbackFactory.filter(), StateFilter(DonateState.donate))
async def handle_process_donation(callback: CallbackQuery, callback_data: DonateCallbackFactory):
...
async def handle_pre_checkout_query(pre_checkout_query: PreCheckoutQuery, state: FSMContext):
    await pre_checkout_query.answer(ok=True)
    await state.update_data(invoice_payload=pre_checkout_query)


@donate_router.callback_query(F.successful_payment)
async def handle_successful_payment(callback: CallbackQuery, state: FSMContext):
...
@donate_router.callback_query(F.data == "back", StateFilter(DonateState.donate))
